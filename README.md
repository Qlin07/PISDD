...
### ⚠️ 本地开发工具注意：MySQL 使用宿主机端口 `13306`

为避开 Windows 动态端口排除范围对 `3306` 的占用，Docker 将 MySQL 映射到宿主机 **`13306`**（容器内仍为 `3306`）。

因此使用本地数据库客户端 / 脚本 / 其他程序直连 MySQL 时，**请连接 `127.0.0.1:13306`**，而非 `3306`：
...
| 端口 | `13306`（**注意不是 3306**） |
| 数据库 | `simplechat` |
| 用户名 | `root` 或 `simplechat` |
| 密码 | `root123456` 或 `simplechat123456` |

服务端由环境变量自动指向 `13306`（见 `server/config/config.go` 的 `MYSQL_PORT`），无需额外手动改动。
