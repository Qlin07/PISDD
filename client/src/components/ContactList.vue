<template>
  <div class="contact-list">
    ...
    <div v-if="pending.length" class="pending">
      <div class="p-title">好友申请</div>
      <div v-for="f in pending" :key="f.id" class="p-item">
        <span class="avatar">{{ '友' }}</span>
        <div class="p-info">
          <div class="p-name">用户 #{{ f.from_id }}</div>
          <div class="p-ops">
            <button class="mini ok" @click="accept(f.from_id)">同意</button>
            <button class="mini no" @click="reject(f.from_id)">拒绝</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <!-- 搜索添加 -->
    <div class="add-bar">
      <input class="input" v-model="kw" placeholder="搜索用户添加好友" @input="doSearch" />
      <div v-if="kw" class="add-results">
        <div v-for="u in results" :key="u.user_id" class="add-item">
          ...
          <button class="mini ok" @click="apply(u.user_id)">添加</button>
        </div>
      </div>
    </div>
    ...
    <div class="f-item" v-for="f in friendUsers" :key="f.user_id" @click="$emit('chat', f)">
      <span class="avatar" :style="avatarStyle(f.avatar_url)">{{ (f.nickname||'?').slice(0,1) }}</span>
      <div class="f-rem">{{ f.remark || f.nickname }}</div>
    </div>
    <div v-if="!friends.length && !kw" class="empty">还没有好友，搜索添加</div>
  </div>
</template>
